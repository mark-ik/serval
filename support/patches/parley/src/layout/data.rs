    ...
    pub(crate) fn finish(&mut self) {
        for run in &self.runs {
            ...
            for cluster in clusters {
                // Join controls affect shaping but are not typographic character
                // units. Applying letter spacing to their hidden zero-advance
                // clusters makes `a<ZWNJ>b` wider than unligated `ab`.
                if matches!(cluster.info.source_char(), '\u{200c}' | '\u{200d}') {
                    continue;
                }
