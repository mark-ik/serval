//! Host-local retained Livery session over Cambium's externally owned DOM.
...
impl OwnedLayout {
    ...
    pub(crate) fn element_scroll(&self) -> &HashMap<NodeId, (f32, f32)> {
        &self.element_scroll
    }
    ...
    pub(crate) fn painted_rect<D: LayoutDom<NodeId = NodeId>>(
        &self,
        dom: &D,
        node: NodeId,
    ...
    pub(crate) fn computed_value(&self, node: NodeId, property: &str) -> Option<String> {
        self.styles.computed_style(node, property)
    }

    pub(crate) fn computed_custom_property(&self, node: NodeId, name: &str) -> Option<String> {
