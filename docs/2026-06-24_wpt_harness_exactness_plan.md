...
- 2026-08-24 — **Reftest fuzzy and reference accounting corrected**: the
  scorer now checks WPT's independent inclusive maximum-channel-delta and
  changed-pixel ranges, honors exact chosen-reference metadata, and retains
  authored lower bounds. Exact maps label known K6 rendering coincidences
  `reference-unverified`; absolute conformance counts verified and unverified
  passes separately. Against the frozen 36,311-file CSS baseline this exposes
  3,650 false passes, gains 19 chosen-reference passes, and makes 157
  reason-only changes. A bounded max-delta-1, 1%-of-raster GPU floor makes two
  complete candidate maps identical. The checked mediaqueries and position
  reftest guards were refreshed and both report `unexpected=0`.
