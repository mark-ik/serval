...
| 17 | Counters, lists, generated content | Livery `content`/`counter-*` cascade and marker boxes | **In progress:** the [scripted-host wave](../design_docs/2026-09-06_scripted_host_capabilities_plan.md) admits inside disc and direct HTML ordered-list decimal markers. Frozen exact maps across 504 list tests show one fail-to-pass (`list-style-position-023`) and zero regressions. Counters, reversed/outside markers and authored generated content remain open. |
